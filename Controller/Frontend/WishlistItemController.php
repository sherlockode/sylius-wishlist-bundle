...
namespace Sherlockode\Sylius\WishlistBundle\Controller\Frontend;
...
class WishlistItemController extends FOSRestController
{
    ...
    public function removeAction(Request $request)
    {
        ...
        $wishlistItem =
            $this->get('sherlockode_wishlist.repository.wishlist_item')->find($request->get('id'));
        ...
        $this->get('sherlockode_wishlist.repository.wishlist_item')->remove($wishlistItem);
        ...
        $this->addFlash(
            'success',
            $this->get('translator')->trans('sherlockode_wishlist.flash.item_removed')
        );
    ...
    public function addAction(Request $request)
    {
        ...
        if ($wishlist->contains($productVariant)) {
            ...
            $this->addFlash(
                'info',
                $this->get('translator')->trans('sherlockode_wishlist.flash.already_on_wishlist')
            );
        ...
        /** @var WishlistItemInterface $wishlistItem */
        $wishlistItem = $this->get('sherlockode_wishlist.factory.wishlist_item')->createNew();
        $wishlistItem->setProductVariant($productVariant);

        $wishlist->addItem($wishlistItem);

        // Persist the wishlist item
        $this->get('sherlockode_wishlist.repository.wishlist_item')->add($wishlistItem);
        ...
        $this->addFlash(
            'success',
            $this->get('translator')->trans('sherlockode_wishlist.flash.item_added')
        );
    ...
    protected function resolveWishlist(Request $request, UserInterface $user)
    {
        ...
        if ($wishlistId = $request->get('wishlistId')) {
            $wishlist = $this->get('sherlockode_wishlist.repository.wishlist')
                             ->findOneBy([
                                 'id'   => $wishlistId,
                                 'user' => $user
                             ]);
        ...
        $wishlist = $this->get('sherlockode_wishlist.repository.wishlist')->getFirstForUser($user);

        // If no wishlist found, create a new one
        if (!$wishlist) {
            $wishlist = $this->get('sherlockode_wishlist.factory.wishlist')->createDefault($user);
            $this->get('sherlockode_wishlist.repository.wishlist')->add($wishlist);
        }
    ...
    protected function resolveProductVariant(Request $request)
    {
        if ($productVariantId = $request->get('productVariantId')) {
            ...
            $productVariant =
                $this->container->get('sherlockode_wishlist.resolver.product_variant_cart')
                                ->resolve($request);
        }
    ...
    protected function redirectToWishlist(WishlistInterface $wishlist)
    {
        ...
        if (false == $this->getParameter('sherlockode_sylius_wishlist.multiple')) {
            return $this->redirectToRoute('sherlockode_frontend_wishlist_first');
        }

        // Redirect back to the wishlist
        return $this->redirectToRoute('sherlockode_frontend_wishlist_show', [
