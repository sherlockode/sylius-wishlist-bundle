...
namespace Sherlockode\Sylius\WishlistBundle\Controller\Account;
...
class WishlistController extends FOSRestController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $wishlists = $this->get('sherlockode_wishlist.repository.wishlist')->findBy([
            'user' => $this->getUser()
        ], [
            'createdAt' => 'asc'
        ]);
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash(
                'success',
                $this->get('translator')->trans('sherlockode_wishlist.flash.updated')
            );

            return $this->redirectToRoute('sherlockode_account_wishlist_edit', [
                'id' => $wishlist->getId()
            ]);
        }
    ...
    public function updateAction(Request $request)
    {
        ...
        if (in_array($request->getMethod(), ['PUT', 'POST'])) {
            if ($form->handleRequest($request)->isValid()) {
                ...
                // Persist changes
                $this->get('sherlockode_wishlist.repository.wishlist')->add($form->getData());
    ...
    public function deleteAction(Request $request)
    {
        ...
        // Remove the wishlist
        $this->get('sherlockode_wishlist.repository.wishlist')->remove($wishlist);
        ...
        $this->addFlash(
            'success',
            $this->get('translator')->trans('sherlockode_wishlist.ui.deleted')
        );

        return $this->redirectToRoute('sherlockode_account_wishlist_index');
    }
}
